...
function activate(context) {
  ...
  vscode2.window.showInformationMessage("\u{1F409} Goku is ready! Run code successfully to see the Kamehameha!");
}
function triggerAnimation(context) {
  const now = Date.now();
  if (now - lastTriggerTime < COOLDOWN_MS) {
    console.log("\u{1F409} Kamehameha on cooldown, skipping...");
    return;
  }
  lastTriggerTime = now;
  const config = vscode2.workspace.getConfiguration("goku");
  const duration = config.get("animationDuration", 4500);
